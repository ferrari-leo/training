...
dir_data = "/Users/IFLM/Desktop/vsc/training/xai_with_python/data/brain_mri"
dir_train = os.path.join(dir_data, "Training")
dir_test = os.path.join(dir_data, "Testing")

# Load data
train_dataset = torchvision.datasets.ImageFolder(root=dir_train)
test_dataset = torchvision.datasets.ImageFolder(root=dir_test)
...
print(f"Batch accuracy: {(labels == outputs).mean()}")
...
def apply_lrp_on_vgg16(model, image):
    ...
    layers = (
        list(model.vgg16._modules["features"])
    ...
    one_hot_output = [
        val if val == max_activation else 0 for val in output_activation[0]
    ]
...
if outputs[image_id] == labels[image_id]:
    print(f"Groundtruth for this image: {pred_label}")
